    ...
    <div className="bg-white border border-dh-border rounded-lg overflow-hidden">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-1.5 py-4 border-t border-dh-border">
          {page > 1 && (
            <Link href={'/dashboard/explorer/my-resources?page=' + (page - 1)} className="dh-btn dh-btn-ghost text-xs px-3">Previous</Link>
          )}
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link key={p} href={'/dashboard/explorer/my-resources?page=' + p}
              className={'dh-btn text-xs min-w-[32px] px-0 ' + (p === page ? 'dh-btn-primary' : 'dh-btn-ghost')}>
              {p}
            </Link>
          ))}
          {page < totalPages && (
            <Link href={'/dashboard/explorer/my-resources?page=' + (page + 1)} className="dh-btn dh-btn-ghost text-xs px-3">Next</Link>
          )}
        </div>
      )}
    </div>
